...
`.github/workflows/release.yml` tags `main` and publishes a GitHub release on every push, choosing the version from the [Conventional Commit](https://www.conventionalcommits.org/) messages since the last tag:

| Commits since the last tag | Next version |
|---|---|
| `feat!:` / `fix!:` or `BREAKING CHANGE:` in the message | major, e.g. `2.0.0` |
| `feat:` | minor, e.g. `1.1.0` |
| `fix:` / `perf:` | patch, e.g. `1.0.1` |
| only `chore` / `docs` / `refactor` / `ci` | no release |

Tags are plain semver with no `v` prefix. Release notes are generated by GitHub from the merged PRs.
