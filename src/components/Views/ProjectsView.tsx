  ...
  return (
    <div className="flex flex-col h-full min-h-[520px] bg-[#002b36] text-[#93a1a1] overflow-hidden select-none font-sans">
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="flex items-center space-x-1 font-mono text-[11px] text-[#839496] bg-[#002b36] px-2 py-1 rounded-md border border-[#2aa198]/30">
            ...
            <span className="text-[#eee8d5] font-bold flex items-center gap-1">
              <Folder className="w-3 h-3 text-[#268bd2]" />
              Projects
            </span>
            {selectedProject && (
              <>
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Finder Sidebar */}
        <div className="w-44 bg-[#073642]/80 border-r border-[#2aa198]/20 p-3 flex flex-col justify-between shrink-0 hidden sm:flex font-sans">
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              <div className="text-[10px] font-bold uppercase tracking-wider text-[#2aa198] px-2 py-1 flex items-center justify-between font-mono">
                <span>Favorites</span>
                <Star className="w-3 h-3 text-[#2aa198]" />
              </div>
              <button
                onClick={() => {
                ...
                className={`w-full text-left px-2.5 py-1.5 rounded-lg text-xs font-medium flex items-center space-x-2 transition-colors cursor-pointer ${
                  activeTag === 'All' && !selectedProject ? 'bg-[#002b36] text-[#eee8d5] font-semibold border border-[#2aa198]/40' : 'text-[#839496] hover:bg-[#002b36]/50 hover:text-[#eee8d5]'
                }`}
              >
            ...
            <div className="space-y-1">
              ...
              {categories.filter(c => c !== 'All').map((cat) => (
                <button
                  ...
                  className={`w-full text-left px-2.5 py-1.5 rounded-lg text-xs font-medium flex items-center space-x-2 transition-colors cursor-pointer ${
                    activeTag === cat ? 'bg-[#002b36] text-[#eee8d5] font-semibold border border-[#2aa198]/40' : 'text-[#839496] hover:bg-[#002b36]/50 hover:text-[#eee8d5]'
                  }`}
                >
        ...
        <div className="flex-1 bg-[#18181a] p-4 sm:p-6 overflow-y-auto custom-scrollbar">
          {selectedProject ? (
            ...
            <div className="space-y-6 text-zinc-200 animate-fadeIn">
              <div className="flex items-center justify-between pb-3 border-b border-zinc-800">
                <button
                  onClick={() => setSelectedProject(null)}
                  className="px-3 py-1 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-xs font-semibold text-zinc-300 flex items-center space-x-1.5 transition-colors cursor-pointer"
                >
                ...
                <div className="flex items-center space-x-2">
                  {selectedProject.githubUrl && (
                    <a
                      ...
                      className="px-3 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-xs font-semibold text-white border border-zinc-700 flex items-center space-x-1.5 transition-colors"
                    >
                  ...
                  {selectedProject.demoUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="px-3 py-1.5 rounded-lg bg-zinc-100 hover:bg-white text-zinc-900 text-xs font-bold flex items-center space-x-1.5 transition-colors shadow-sm"
                    >
              ...
              <div className="p-6 rounded-2xl bg-zinc-900 border border-zinc-800 space-y-3">
                ...
                {selectedProject.metrics && (
                  <div className="inline-flex items-center space-x-2 px-3 py-1.5 rounded-xl bg-zinc-800 border border-zinc-700 text-zinc-300 text-xs font-semibold">
              ...
              <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {selectedProject.technologies.map((tech, idx) => (
                    <span key={idx} className="px-2.5 py-1 rounded-md bg-zinc-800 text-xs text-zinc-200 font-mono border border-zinc-700">
                      {tech}
                    </span>
                  ))}
                </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 space-y-2">
                  <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-wider flex items-center space-x-1.5 font-mono">
                ...
                <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 space-y-2">
                  <h3 className="text-xs font-bold text-zinc-200 uppercase tracking-wider flex items-center space-x-1.5 font-mono">
              ...
              {filteredProjects.map((project) => (
                ...
                >
                  <div className="relative">
                    <div className="w-16 h-16 rounded-2xl bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-200 group-hover:scale-105 group-hover:border-zinc-700 transition-transform shadow-md">
                      <Folder className="w-9 h-9 fill-zinc-800/80" />
                    </div>
                    <span className="absolute -bottom-1 -right-1 px-1.5 py-0.5 rounded text-[9px] font-mono font-bold bg-zinc-950 border border-zinc-800 text-zinc-400">
                      {project.category}
                    </span>
                  </div>

                  <div>
                    <h3 className="text-xs font-bold text-white group-hover:text-zinc-200 line-clamp-1">
                      {project.title}
                    </h3>
            ...
            <div className="bg-zinc-900/80 rounded-xl border border-zinc-800 overflow-hidden divide-y divide-zinc-800 text-xs">
              ...
              {filteredProjects.map((project) => (
                ...
                >
                  ...
                  <div className="col-span-2 text-right font-mono text-[10px] text-zinc-400 truncate">
                    {project.technologies.slice(0, 2).join(', ')}
                  </div>
                </div>
              ))}
